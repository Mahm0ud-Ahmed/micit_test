...
class AppException implements Exception {
  ...
  ErrorModel get handleError {
    ...
    if (error is AppException && error.error is AppError) {
      errorModel = ErrorModel(statusMessage: (error.error as AppError).statusMessage);
    } else if (error is AppException && error.error is DioException) {
      ...
      switch (dioError.type) {
        ...
        case DioExceptionType.badResponse:
          if (dioError.response?.statusCode != null && dioError.response != null) {
            ...
            errorModel.statusCode == null
                ? errorModel = errorModel.copyWith(statusCode: dioError.response?.statusCode)
                : null;
          } else {
    ...
    } else if (error is AppException && error.error.error is DatabaseException) {
      final DatabaseException dbError = error.error.error;
      if(dbError is SqfliteDatabaseException){
        errorModel = ErrorModel(statusMessage: dbError.message);
      }
    } else if (error is TypeError && error.toString().contains('is not a subtype of type')) {
      AppLogger.logError(error);
      errorModel =
          ErrorModel(statusMessage: S.current.error_occurred_while_processing_the_data_received_from_the_server_message);
    } else {
